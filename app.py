...
mathematics = st.number_input("Mathematics Marks (out of 200)", min_value=0, max_value=200, step=1)

category = st.selectbox("Category", ["OC", "BC", "MBC", "SC", "ST"])

# ✅ Calculate TNEA Cutoff Score
cutoff_score = (mathematics * 100 / 200) + (physics * 50 / 100) + (chemistry * 50 / 100)

# 🔹 Display Cutoff Score
st.markdown(f"### ✅ Cut Off = **{cutoff_score:.2f} / 200**")

# 🔍 Predict College
if st.button("Predict College"):
    category_encoded = label_encoder_category.transform([category])[0]
    input_data = np.array([[physics, chemistry, mathematics, cutoff_score, category_encoded]])
